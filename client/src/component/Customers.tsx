...
import { Spinner, CheckBox, Layer, Box, Text, DataTable, Card, CardHeader, CardBody, Button } from 'grommet';
        ...
        <Box>
            <DataTable
                columns={[
                    ...
                    },
                    {
                        ...
                        render: customer => (
                            <FullOrdersPrice customerId={customer.id} />
                        )
                    },
                ]}
                data={data.customers}
                fill
                onClickRow={({ datum }) => { setId(datum.id); setShow(true) }}
                pin
                sortable
            />
            {show && (<Layer
            ...
            >
                <Card background="light-1">
                    <CardHeader className='card' pad="medium">Orders</CardHeader>
                    <CardBody pad="medium">
                        <Orders customerId={id} />
                    </CardBody >
                    <Button label="close" margin='medium' onClick={() => { setShow(false) }} />
                </Card >
            </Layer>
            )}
        </Box>
